...
`2026 경진대회·초급과정 신청 접수 관리대장`에서 아래 접수·운영 탭을 사용합니다.
...
- `운영설정`: 경진대회, 초급과정별 신규 접수, 신청 확인·변경·취소, 장학금 신청의 운영 여부
...
신규 접수 기능을 추가한 뒤에는 `setupApplicationSheets`를 실행하여 접수 탭의 헤더와
드롭다운 및 별도 관리대장 편집 트리거를 확인하고, `runBenefitsSmokeTest`를 실행한 다음
웹앱을 새 버전으로 배포합니다. `setupBenefitsSheet`를 실행해도 별도 관리대장 설정이 함께 실행됩니다.

최초 배포 이후에는 코드의 상수를 수정하지 않습니다. `운영설정` 탭의 `운영 여부` 체크박스로
각 과정을 열거나 닫습니다. 체크 상태는 홈페이지의 모집 상태와 접수 버튼, 직접 접수 URL,
Apps Script의 실제 저장 허용 여부에 함께 반영됩니다. `신청 확인·변경·취소`와 `장학금 신청`도
같은 탭에서 별도로 제어합니다. 이수증 발급은 운영설정과 관계없이 이용할 수 있습니다.

`APPLICATIONS_OPEN`과 `SCHOLARSHIP_APPLICATIONS_OPEN`은 운영설정 탭에 접근하지 못할 때만
사용하는 안전 기본값이며, 둘 다 `false`를 유지합니다. 운영설정의 기능명·설명·설정 키는 변경하지
말고 `운영 여부` 체크박스만 수정합니다.
...
실제 접수를 열기 전 테스트할 때는 `운영설정` 탭에서 해당 과정의 체크를 해제합니다. 테스트 키가
